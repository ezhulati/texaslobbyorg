-- Update search_lobbyists function to only return approved profiles
-- This ensures rejected and pending profiles are hidden from public marketplace
...
CREATE OR REPLACE FUNCTION search_lobbyists(
  ...
  client_filters TEXT[] DEFAULT NULL,
...
BEGIN
  ...
  WHERE
    l.is_active = true
    AND l.approval_status = 'approved'  -- NEW: Only show approved profiles
...
COMMENT ON FUNCTION search_lobbyists IS 'Search lobbyists with filters for text, cities, subjects, tier, and clients. Only returns approved profiles. Uses full-text search with ranking.';
